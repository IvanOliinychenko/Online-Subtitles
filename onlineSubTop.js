...
chrome.windows.getCurrent({ populate: true }).then(window => {
  tabUrl = extractDomain(window.tabs.find(tab => tab.active).url)
})
...
onoffOnlineSubOnPage.addEventListener('change', onoffOnlineSubOnPageFunc, false);
onoffOnlineSub.addEventListener('change', onoffOnlineSubFunc, false);
